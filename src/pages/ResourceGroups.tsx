...
export default function ResourceGroups() {
  ...
  const handleCreateResourceGroup = async (
    ...
    try {
      await createResourceGroup(name, description, region_id);
      await loadData();
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      // Update the local state by removing the deleted resource group
      setResourceGroups(prevGroups => 
        prevGroups.filter(group => group.id !== deleteConfirmation.resourceGroup?.id)
      );
      // Clear the selected group if it was the one that was deleted
